...
execute at @e[type=minecraft:falling_block,nbt={BlockState: {Name: "minecraft:anvil"}}] positioned ~ ~-1 ~ as @e[type=item_frame,limit=1,distance=..1,sort=nearest,nbt={Facing: 1b},tag=!saved] run tag @e[type=marker,limit=1,sort=nearest,tag=reforge] add used
...
execute as @e[type=item_frame,nbt={Item: {tag: {reforged: 0b}}}] run data modify entity @s Item.tag merge from entity @e[type=minecraft:marker,limit=1,sort=nearest,tag=reforge] data.Item.tag
